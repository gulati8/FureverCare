...
// Ensure image uploads directory exists
const imageUploadsDir = path.join(process.cwd(), config.imageUpload.uploadDir);
...
router.post('/:petId/photo-import/upload', authenticate, pdfUploadRateLimit, upload.single('image'), async (req: AuthRequest, res: Response) => {
  try {
