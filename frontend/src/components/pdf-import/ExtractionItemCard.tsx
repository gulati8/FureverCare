...
export function ExtractionItemCard({
  ...
  return (
    <div className={`rounded-lg border ${cardColor} overflow-hidden ${isDisabled ? 'opacity-60' : ''}`}>
      ...
      {/* Duplicate warning */}
      {duplicateOf && (
        <div className="mx-3 mb-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-md flex items-center gap-2 text-sm text-amber-800">
          <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
          <span>Existing medication found: <strong>{duplicateOf}</strong>. Approving will update the existing record instead of creating a duplicate.</span>
        </div>
      )}
