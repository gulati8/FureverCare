...
interface SourceDocumentLinkProps {
  ...
  recordType: string;
  recordId: number;
  onNavigateToReview?: (uploadId: number, extractionItemId: number) => void;
}

export default function SourceDocumentLink({ petId, recordType, recordId, onNavigateToReview }: SourceDocumentLinkProps) {
  const [source, setSource] = useState<{ source: string; upload_id: number | null; extraction_item_id?: number | null; filename: string | null } | null>(null);
  ...
  return (
    <button
      type="button"
      onClick={() => {
        if (source.upload_id && source.extraction_item_id && onNavigateToReview) {
    ...
    >
      ...
      <span className="truncate max-w-[180px]">View source document</span>
    </button>
  );
}
