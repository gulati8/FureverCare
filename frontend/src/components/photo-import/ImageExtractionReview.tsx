...
import { photoImportApi, ImageUpload, ImageExtractionWithItems } from '../../api/client';
import { ExtractionItemCard } from '../pdf-import/ExtractionItemCard';

interface ImageExtractionReviewProps {
  petId: number;
  upload: ImageUpload;
  onBack: () => void;
...
export function ImageExtractionReview({ petId, upload, onBack, onApprovalComplete }: ImageExtractionReviewProps) {
  const { token } = useAuth();
  const [extraction, setExtraction] = useState<ImageExtractionWithItems | null>(null);
  ...
  const loadExtraction = async () => {
    ...
    try {
      const data = await photoImportApi.getExtraction(petId, upload.id, token);
      setExtraction(data);
  ...
  const handleModifyItem = async (itemId: number, modifiedData: Record<string, any>) => {
    ...
    try {
      const updatedItem = await photoImportApi.editItem(petId, itemId, modifiedData, token);
  ...
  const handleApprove = async () => {
    ...
    try {
      const result = await photoImportApi.approveItems(petId, upload.id, Array.from(selectedIds), token);
      ...
      const updatedExtraction = await photoImportApi.getExtraction(petId, upload.id, token);
  ...
  const handleReject = async () => {
    ...
    try {
      await photoImportApi.rejectItems(petId, upload.id, Array.from(selectedIds), token);
      await loadExtraction();
      setSelectedIds(new Set());
    } catch (err: any) {
